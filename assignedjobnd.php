<?php
session_start();
 if($_SESSION['technician_rank']!="2nd Leader" ){
  header("location:assignedjob.php");
 }
?>
	...
	<div class="column">
		...
		<?php
			...
			job_assign !='Storekeeper' AND job_assign IS NOT NULL AND  job_status = '' AND job_cancel='' AND accessories_required =''
				OR
			job_assign !='Storekeeper' AND job_assign IS NOT NULL AND  job_status IS NULL AND job_cancel IS NULL AND accessories_required IS NULL
				OR
			job_assign !='Storekeeper' AND job_assign IS NOT NULL AND  job_status = '' AND job_cancel='' AND accessories_required ='No'
			OR
			job_assign !='Storekeeper' AND job_assign IS NOT NULL AND  job_status IS NULL AND job_cancel IS NULL AND accessories_required ='No'
    ...
    <div class="column">
        <p class="column-title" id="joblisting"><b>Doing</b></p>
			<?php
				...
				OR
				job_assign !='Storekeeper' AND job_assign IS NOT NULL AND job_status = 'Doing' AND job_cancel IS NULL AND accessories_required IS NULL
				OR
				job_assign !='Storekeeper' AND job_assign IS NOT NULL AND job_status = 'Doing' AND job_cancel='' AND accessories_required ='No'
				OR
				job_assign !='Storekeeper' AND job_assign IS NOT NULL AND job_status = 'Doing' AND job_cancel IS NULL AND accessories_required ='No'
				ORDER BY jobregisterlastmodify_at DESC LIMIT 50";
