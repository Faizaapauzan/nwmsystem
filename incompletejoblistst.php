...
<html lang="en">
    ...
    <body>
            ...
            <div class="Box card" style="margin-bottom: 30px;" id="livesearch">
                <div class="card-body">
                    <div class="clearfix mb-2">
                        <div class="float-start"><h6 class="fw-bold" style="color: #F28C28;">Incomplete</h6></div>
                        <?php
                            
                            include 'dbconnect.php';
                            
                            $numRow = "SELECT * FROM job_register WHERE (job_status = 'Incomplete' AND job_cancel = '' AND staff_position != 'Storekeeper')
                                       OR (job_status = 'Incomplete' AND job_cancel IS NULL AND staff_position != 'Storekeeper')";

                            $numRow_run = mysqli_query($conn, $numRow);
                                    
                            if ($row_Total = mysqli_num_rows($numRow_run)) {
                              echo '<div class="float-end"><h6 class="fw-bold" style="color: #F28C28;">Total Job: '. $row_Total .'</h6></div> ';
                            }
                            
                            else {
                              echo '<div class="float-end"><h6 class="fw-bold" style="color: #F28C28;">No Record</h6></div>';
                            }
                        ?>
                    </div>
                    ...
                    <?php
                        ...
                        $results = $conn->query("SELECT * FROM job_register WHERE (job_status = 'Incomplete' AND job_cancel = '' AND staff_position != 'Storekeeper')
                                                 OR (job_status = 'Incomplete' AND job_cancel IS NULL AND staff_position != 'Storekeeper')
        ...
        <!-- Popup Modal -->
        <div class="modal fade" id="incompleteJobModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    ...
                                    <div class="card mb-3">
                                        <div class="card-body">
                                            ...
                                            <form id="pendingJob">
                                                ...
                                                <div id="pandingJobMessage"></div>
                                                
                                                <script>
                                                    $(document).ready(function() {
                                                        $.fn.select2.amd.require(['select2/compat/matcher'], function (oldMatcher) {
                                                            $("#job_assign").select2({
                                                                dropdownParent: $('#pendingJob'),
        ...
        <script>
            ...
            // Update Job Assign
            $(document).on('submit', '#pendingJob', function (e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (res.status === 200) {
                            $('#pandingJobMessage').html('<p class="fw-bold" style="text-align: center; color: green; display:block;">' + res.message + '</p>');
                            
                            setTimeout(function () {
                                hideElementById("pandingJobMessage");
                            }, 2000);
                        } 
                        
                        else if (res.status === 500) {
                            $('#pandingJobMessage').html('<p class="fw-bold" style="text-align: center; color: red; display:block;">' + res.message + '</p>');
                    ...
                    error: function (xhr, status, error) {
                        $('#pandingJobMessage').html('<p class="fw-bold" style="text-align: center; color: red; display:block;">An error occurred while processing your request.</p>');
