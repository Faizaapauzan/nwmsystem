...
<html lang="en">
    <head>
        ...
        <title>Technician Unassigned Job</title>
    ...
    <body>
            ...
            <div class="Box card" style="margin-bottom: 30px;" id="livesearch">
                <div class="card-body">
                    <div class="clearfix mb-2">
                        <div class="float-start"><h6 class="fw-bold" style="color: #081d45;">Unassign</h6></div>
                        <?php
                    ...
                    ?>
                    <div class="Job card mb-3" data-bs-toggle="modal" data-bs-target="#unassignedJobModal" data-jobRegisterID="<?php echo $row['jobregister_id']; ?>">
                            ...
                            <?php
								if($row['support'] != "" || $row['support'] != NULL) {
									echo'<div class="d-grid justify-content-end">
										 	<h6 class="badge bg-secondary text-wrap fw-bold ms-3" style="font-size: 0.8rem; width: 6rem;">'.$row['support'].'</h6>
                            			 </div>';
								}
                            ?>
                        </div>
        ...
        <!-- Popup Modal -->
        <div class="modal fade" id="unassignedJobModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            ...
                            <div class="tab-pane" id="pills-JobAssign" role="tabpanel" aria-labelledby="pills-JobAssign-tab">
                                <div class="container">
                                    <form id="unassignedJob">
                                        ...
                                        <div id="unassignedMessage"></div>

                                        <script>
                                            $(document).ready(function() {
                                                $.fn.select2.amd.require(['select2/compat/matcher'], function (oldMatcher) {
                                                    $("#job_assign").select2({
                                                        dropdownParent: $('#unassignedJob'),
        ...
        <script>
            ...
            // Job Assign Data
            function fetchUnassignedJobData(jobregister_id) {
                $.ajax({
                    type: "GET",
                    url: "technicianPopupModalAllIndex.php?jobregister_id=" + jobregister_id,
            ...
            $(document).on('click', '.Job', function () {
                ...
                fetchUnassignedJobData(jobregister_id)
                
                $('#unassignedJobModal').modal('show');
            });
            ...
            // Update Job Assign
            $(document).on('submit', '#unassignedJob', function (e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (res.status === 200) {
                            $('#unassignedMessage').html('<p class="fw-bold" style="text-align: center; color: green; display:block;">' + res.message + '</p>');
                            
                            setTimeout(function () {
                                hideElementById("unassignedMessage");
                            }, 2000);
                        } 
                        
                        else if (res.status === 500) {
                            $('#unassignedMessage').html('<p class="fw-bold" style="text-align: center; color: red; display:block;">' + res.message + '</p>');
                    ...
                    error: function (xhr, status, error) {
                        $('#unassignedMessage').html('<p class="fw-bold" style="text-align: center; color: red; display:block;">An error occurred while processing your request.</p>');
